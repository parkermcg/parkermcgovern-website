...
- **Search Console property type unconfirmed.** The canonical host is now `www.parkermcgovern.ca`. A URL-prefix property for the apex alone would report on almost nothing, since the apex 308s to www. A Domain property covers both and is the safer choice. Confirm which exists before relying on the data.
